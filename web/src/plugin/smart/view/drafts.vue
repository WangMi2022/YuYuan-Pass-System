  ...
  <main class="na-page na-page--list smart-drafts-page">
    ...
    <div class="draft-layout">
      ...
      <section class="na-panel list-panel">
        <header class="panel-header"><div><h2>待确认草稿</h2><p>确认日程会写入个人日历；确认业务单只创建草稿，不会自动提交。</p></div><el-select v-model="draftType" clearable placeholder="全部类型" @change="filterDrafts"><el-option label="日程" value="schedule" /><el-option label="业务单" value="operation" /></el-select></header>
        ...
        <div v-if="total > 10" class="na-pagination">
          <el-pagination
...
async function loadDrafts() { loading.value = true; try { const res = await getSmartDrafts({ paged: true, page: page.value, pageSize: pageSize.value, draftType: draftType.value || undefined }); if (res.code === 0) { drafts.value = res.data?.list || []; total.value = Number(res.data?.total || 0) } } finally { loading.value = false; loaded.value = true } }
function filterDrafts() { page.value = 1; return loadDrafts() }
function resetDraftPage() { page.value = 1 }
async function extract() { if (!announcementId.value) return; creating.value = true; try { const res = await extractAnnouncementSchedule({ announcementId: announcementId.value }); if (res.code === 0) { ElMessage.success(res.msg || '草稿已生成'); await filterDrafts() } else ElMessage.error(res.msg || '提取失败') } finally { creating.value = false } }
