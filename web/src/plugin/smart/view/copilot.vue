<template>
  <main class="na-page smart-copilot-page">
    ...
    <div class="copilot-layout">
      ...
      <section class="na-panel chat-panel">
        <div class="chat-scroll" aria-live="polite">
          ...
          <div v-if="!messages.length" class="empty-chat">
            <span class="empty-chat-icon"><el-icon><ChatDotRound /></el-icon></span>
            <div class="empty-chat-copy">
              <strong>查询资产和运营数据</strong>
              <span>选择一个常用问题，或在下方输入只读业务问题。</span>
            </div>
            <div class="question-suggestions" aria-label="常用问题">
      ...
      <aside class="na-panel tools-panel"><header class="panel-header"><h2>可用 Tool</h2></header><div v-for="item in tools" :key="item.name" class="tool-item"><el-icon><CircleCheck /></el-icon><div><strong>{{ item.name }}</strong><span>{{ item.description }}</span></div></div><AppEmptyState v-if="!tools.length" compact title="暂无可用 Tool" description="当前角色没有可调用的只读业务工具。" /></aside>
    </div>
  </main>
</template>
...
const suggestedQuestions = ['未来 30 天哪些资产质保到期？', '当前有哪些高风险资产？', '待核对发票有多少张？', '今天有哪些日程和未读公告？']
...
function chooseQuestion(value) { question.value = value }
...
.copilot-layout { display: grid; grid-template-columns: 220px minmax(0, 1fr) 250px; align-items: start; gap: 14px; }.na-panel { min-width: 0; padding: 16px; border: 1px solid var(--na-border); border-radius: 8px; background: var(--na-card); box-shadow: var(--na-shadow-sm); }.panel-header { display: flex; align-items: center; justify-content: space-between; gap: 8px; margin-bottom: 12px; }.panel-header h2 { margin: 0; font-size: .95rem; }.session-panel, .tools-panel { max-height: calc(100vh - 170px); overflow-y: auto; }.session-row { display: grid; grid-template-columns: minmax(0, 1fr) 30px; align-items: center; border-radius: 6px; }.session-row:hover, .session-row.active { background: var(--na-surface-muted); }.session-item { display: flex; min-width: 0; flex-direction: column; gap: 4px; padding: 10px; border: 0; background: transparent; color: var(--na-foreground); text-align: left; cursor: pointer; }.session-item small { color: var(--na-muted-foreground); }.session-delete { width: 28px; height: 28px; margin: 0; color: var(--na-muted-foreground); }.session-delete:hover { color: var(--el-color-danger); }.chat-panel { display: flex; min-height: max(620px, calc(100vh - 170px)); flex-direction: column; }.chat-scroll { flex: 1; min-height: 0; overflow: auto; padding: 4px 2px 16px; }.message { max-width: 82%; margin: 0 0 12px; padding: 10px 12px; border-radius: 8px; }.message-user { margin-left: auto; background: var(--el-color-primary-light-9); }.message-assistant { border: 1px solid var(--na-border); background: var(--na-surface-muted); }.message-role { display: flex; justify-content: space-between; margin-bottom: 5px; color: var(--na-muted-foreground); font-size: .72rem; }.message-role span { color: var(--el-color-primary); }.message p, .result-answer { margin: 0; line-height: 1.6; white-space: pre-wrap; }.message-citations { padding-top: 8px; border-top: 1px solid var(--na-border); }.empty-chat { display: grid; min-height: 430px; grid-template-columns: 52px minmax(0, 1fr); align-content: center; gap: 6px 14px; padding: 32px; color: var(--na-muted-foreground); }.empty-chat-icon { display: grid; width: 52px; height: 52px; grid-row: 1 / span 2; place-items: center; border-radius: 8px; background: var(--na-primary-soft); color: var(--na-primary); font-size: 24px; }.empty-chat-copy { display: flex; min-width: 0; flex-direction: column; gap: 4px; }.empty-chat strong { color: var(--na-foreground); font-size: 1rem; }.empty-chat-copy span { font-size: .75rem; }.question-suggestions { display: grid; grid-column: 1 / -1; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px; margin-top: 18px; }.question-suggestions button { display: flex; min-width: 0; min-height: 48px; align-items: center; justify-content: space-between; gap: 10px; padding: 9px 12px; border: 1px solid var(--na-border); border-radius: 7px; background: var(--na-card); color: var(--na-foreground); font: inherit; font-size: .75rem; text-align: left; cursor: pointer; }.question-suggestions button:hover, .question-suggestions button:focus-visible { border-color: var(--na-primary); background: var(--na-primary-soft); outline: none; }.question-suggestions button span { min-width: 0; overflow-wrap: anywhere; }.question-suggestions button .el-icon { flex: 0 0 auto; color: var(--na-primary); }.result-card { margin: 4px 0 14px; padding: 12px; border: 1px solid var(--na-border); border-radius: 8px; }.result-meta, .citations { display: flex; align-items: center; flex-wrap: wrap; gap: 8px; margin-bottom: 10px; color: var(--na-muted-foreground); font-size: .75rem; }.citations { margin: 12px 0 0; }.citations .el-button { max-width: 100%; margin: 0; white-space: normal; text-align: left; }.composer { display: flex; align-items: flex-end; gap: 10px; padding-top: 12px; border-top: 1px solid var(--na-border); }.composer .el-input { flex: 1; }.tools-panel { align-self: start; }.tool-item { display: flex; gap: 8px; padding: 9px 0; border-bottom: 1px solid var(--na-border); }.tool-item:last-child { border-bottom: 0; }.tool-item .el-icon { flex: 0 0 auto; color: var(--el-color-success); }.tool-item div { display: flex; min-width: 0; flex-direction: column; gap: 3px; }.tool-item strong { font-size: .78rem; }.tool-item span { color: var(--na-muted-foreground); font-size: .72rem; line-height: 1.4; }
@media (max-width: 1100px) { .copilot-layout { grid-template-columns: 190px minmax(0, 1fr); }.tools-panel { display: none; } }
@media (max-width: 700px) { .copilot-layout { display: flex; flex-direction: column; min-height: 0; }.session-panel { max-height: 180px; overflow: auto; }.chat-panel { min-height: calc(100vh - 240px); }.empty-chat { min-height: 360px; grid-template-columns: 44px minmax(0, 1fr); padding: 24px 14px; }.empty-chat-icon { width: 44px; height: 44px; }.question-suggestions { grid-template-columns: 1fr; }.composer { flex-direction: column; align-items: stretch; }.composer .el-button { width: 100%; }.message { max-width: 94%; } }
</style>
