...
const CHUNK_LOAD_RELOAD_KEY = 'gva:chunk-load-reload-at'

const isDynamicImportLoadError = (error) => {
  const message = String(error?.message || error || '')
  return /Failed to fetch dynamically imported module|Importing a module script failed|error loading dynamically imported module|Expected a JavaScript-or-Wasm module script/i.test(message)
}

const reloadForFreshAssets = (error) => {
  if (!isDynamicImportLoadError(error)) return false
  const lastReloadAt = Number(sessionStorage.getItem(CHUNK_LOAD_RELOAD_KEY) || 0)
  if (Date.now() - lastReloadAt < 10000) return false
...
router.onError((error) => {
  if (reloadForFreshAssets(error)) return
  console.error('Router error:', error)
  Nprogress.remove()
})

window.addEventListener('unhandledrejection', (event) => {
  if (reloadForFreshAssets(event.reason)) {
