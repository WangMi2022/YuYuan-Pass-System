...
- 侧边栏重新整理图标、父子层级、当前项标记和折叠状态，底部提示卡改为紧凑布局。
- Docker 健康检查增加启动等待重试，避免后端首次迁移期间产生错误告警。

本版本已通过 Go 测试、Vite 生产构建、六类位置接口 CRUD、十二组业务位置草稿回显及 Docker 容器健康检查。
...
- 全站采用现代 SaaS 简约视觉（zinc 中性灰阶 + indigo 默认主色），设计令牌统一驱动业务页与系统管理页，亮/暗双主题完整覆盖。
- 首页工作台提供问候横幅、资产概览、权限感知的快捷入口与最新公告；横幅与资产大屏内置 Three.js 粒子特效，颜色跟随主题色与暗色模式实时联动，并支持 reduced-motion 降级。
...
- 侧边栏采用清晰的父子层级、图标容器和当前项指示条；左下提示卡保持紧凑，折叠、移动端或低高度窗口会自动隐藏。
...
- 左侧菜单允许多个父菜单同时展开，切换业务模块不会自动收起之前展开的菜单。
