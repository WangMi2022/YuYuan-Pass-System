...
M3 当前 11 张历史/存量发票尚未产生字段级复核样本，因此字段修改率只能从后续真实人工复核开始准确累计；M4 Prompt V3 兼容 `productName`、`manufacturer`、`warrantyMonths` 三个受控别名，最终统一归一为标准字段，未知字段仍被 Schema 拒绝。M3 质量看板的 `1440×1000`、`900×900` 已完成视觉检查，`390×844` 已完成 DOM/几何验证，三档均无页面级横向溢出。M4 空态页面的 `1440×1000`、`900×900`、`390×844` 三档 DOM/几何验证均通过；由于生产任务列表为空，有任务抽屉仍未实测。浏览器 CDP 截图持续超时，因此未取得截图的档位不能写成完整视觉截图验收通过。详细证据见 [M3/M4 生产响应式验收记录](docs/M3-M4-RESPONSIVE-VALIDATION.md)。后续建议按以下顺序推进：

1. 持续用真实数据校准 M2 风险阈值、M3 质量口径和 M4 字段置信度。
2. 经单独确认后创建并清理一次生产测试识别任务，补齐 M4 有任务抽屉三档验收；截图能力恢复后补充 M3 手机档与 M4 三档归档图片。
