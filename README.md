...
当前已经具备资产生命周期、资产风险治理、发票处理、文档协作、个人日程、公告通知、权限审计、统一 AI Gateway 和 Compose 交付闭环。智能化路线 M0 安全门禁与 M1 Gateway 均已通过生产验收；M2 资产风险中心已完成开发、插件测试和生产构建验证，正在等待生产验收。后续建议按以下顺序推进：

1. 完成 M2 生产发布、规则阈值校准和权限验收。
2. M4 智能资产建档 MVP。
3. M3 发票修正差异与质量看板。
