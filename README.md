...
- 当前编排实现位于 `server/plugin/smart/service`，采用规则规划器、可选模型条件解析和受控 Tool Registry，不依赖 LangGraph 运行时。资产查询支持日期、金额、状态、部门、保管人的组合筛选、排序和统计，详见 [条件查询说明](docs/SMART-ASSISTANT-P0-P3.md#资产条件查询)。
...
业务助手的问答流程是“问题 → 规则或模型解析 → 条件校验与受控 Tool → 实时业务查询 → 引用和审计”。资产条件查询直接展示实际条件和数据库统计；“过期”等含糊问法先澄清，未能完整解析的条件不会变成零结果。其他业务工具保留可选模型润色。日程、未读公告、资产和发票等问题必须依赖对应 Tool 的权限；模型不可直接读取数据库。当前编排位于 `server/plugin/smart/service`，不依赖 LangGraph 运行时。
