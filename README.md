...
当前已经具备资产生命周期、资产风险治理、发票处理、文档协作、个人日程、公告通知、权限审计、统一 AI Gateway 和 Compose 交付闭环。智能化路线 M0 安全门禁与 M1 Gateway 均已通过生产验收；M2 资产风险中心已于 2026-08-14 上线，发布验收 8/8 通过，四张风险表与 17 条规则已核验，启动后的定时扫描成功，匿名接口均被拒绝访问。多端页面视觉验收待在稳定的管理员浏览器会话中补做。后续建议按以下顺序推进：

1. 基于真实命中数据校准 M2 规则阈值，并补做多端页面视觉验收。
2. M4 智能资产建档 MVP。
3. M3 发票修正差异与质量看板。
