...
当前已经具备资产生命周期、资产风险治理、发票处理、文档协作、个人日程、公告通知、权限审计、统一 AI Gateway 和 Compose 交付闭环。智能化路线 M0-M7 首版已完成：M3 的 `invoice_review_corrections`、识别质量扩展字段和 5 个 `/invoiceQuality/*` 接口已通过生产只读验收；M4 的 `asset-draft` Prompt V3、真实 Vision 调用、结构化草稿和任务/图片清理已通过生产验收；M5-M7 已加入 `smart` 插件、动态菜单、受控 Tool、用户日报和人工确认草稿链路。M3/M4 响应式修复已随生产版本 `1a5e0f1e4829940cdd7886db6428c413753f74d5` 发布，发布门禁 8/8 通过，Server/Web 容器运行正常。
...
3. 部署 M5-M7，验证不同角色的数据范围、日报指标口径和草稿确认边界。
4. 在生产环境验证定时日报站内/邮件投递、公告编辑器入口、资产候选和底层业务权限二次校验。
