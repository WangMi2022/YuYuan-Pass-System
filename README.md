...
当前已经具备资产生命周期、资产风险治理、发票处理、文档协作、个人日程、公告通知、权限审计、统一 AI Gateway 和 Compose 交付闭环。智能化路线 M0 安全门禁、M1 Gateway 和 M2 资产风险中心均已通过生产验收。M3 发票识别质量闭环与 M4 智能资产建档已完成代码、自动化测试、前端 lint 和生产构建，正在执行多尺寸页面与生产发布验收。M4 真实识别依赖生产 AI Gateway、有效 Vision Provider/模型/API Key，以及运行配置中的 `allow-vision-modules: [asset]`。后续建议按以下顺序推进：

1. 基于真实命中数据校准 M2 规则阈值和责任分配方式。
2. 完成 M3/M4 生产验收并用真实样本校准字段置信度与质量口径。
3. M5 只读业务助手与受控查询 Tool。
4. M6 智能管理日报。
5. 业务插件 Swagger 全覆盖、CI 发布门禁、资产盘点与标签/二维码。
