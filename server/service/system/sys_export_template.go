...
// StoreExcel writes a generated Excel workbook to the currently configured
// OSS provider. The original download response remains unchanged.
func (sysExportTemplateService *SysExportTemplateService) StoreExcel(filename string, file *bytes.Buffer) (fileURL, key string, err error) {
	if file == nil {
		return "", "", errors.New("Excel文件内容不能为空")
	}
	return upload.UploadBytes(upload.NewOss(), filename, file.Bytes())
}

// ImportExcelAndStore archives the uploaded workbook in OSS and then imports
// it. If importing fails, the just-created OSS object is removed.
func (sysExportTemplateService *SysExportTemplateService) ImportExcelAndStore(templateID string, file *multipart.FileHeader) (fileURL, key string, err error) {
	if file == nil {
		return "", "", errors.New("Excel文件不能为空")
	}

	storage := upload.NewOss()
	fileURL, key, err = storage.UploadFile(file)
	if err != nil {
		return "", "", fmt.Errorf("Excel上传OSS失败: %w", err)
	}

	if err = sysExportTemplateService.ImportExcel(templateID, file); err != nil {
		if cleanupErr := storage.DeleteFile(key); cleanupErr != nil {
			return "", "", fmt.Errorf("%w（OSS临时文件清理失败: %v）", err, cleanupErr)
		}
		return "", "", err
	}
