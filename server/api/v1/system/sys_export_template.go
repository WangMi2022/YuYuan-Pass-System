...
import (
	...
	"github.com/WangMi2022/mit-assets-admin/server/service"
...
func (sysExportTemplateApi *SysExportTemplateApi) ExportExcelByToken(c *gin.Context) {
	...
	// 导出并归档到当前 OSS，下载响应保持兼容。
	if file, name, err := sysExportTemplateService.ExportExcel(templateID, queryParams); err != nil {
		global.GVA_LOG.Error("获取失败!", zap.Error(err))
		response.FailWithMessage("获取失败", c)
	} else {
		filename := name + utils.RandomString(6) + ".xlsx"
		fileURL, key, uploadErr := sysExportTemplateService.StoreExcel(filename, file)
		if uploadErr != nil {
			global.GVA_LOG.Error("Excel上传OSS失败!", zap.Error(uploadErr))
			response.FailWithMessage("Excel上传OSS失败", c)
			return
		}
		c.Header("X-Excel-OSS-URL", url.QueryEscape(fileURL))
		c.Header("X-Excel-OSS-Key", url.QueryEscape(key))
...
func (sysExportTemplateApi *SysExportTemplateApi) ExportTemplateByToken(c *gin.Context) {
	...
	// 导出模板并归档到当前 OSS，下载响应保持兼容。
	if file, name, err := sysExportTemplateService.ExportTemplate(templateID); err != nil {
		global.GVA_LOG.Error("获取失败!", zap.Error(err))
		response.FailWithMessage("获取失败", c)
	} else {
		filename := name + "模板.xlsx"
		ossFilename := name + "模板_" + utils.RandomString(6) + ".xlsx"
		fileURL, key, uploadErr := sysExportTemplateService.StoreExcel(ossFilename, file)
		if uploadErr != nil {
			global.GVA_LOG.Error("Excel模板上传OSS失败!", zap.Error(uploadErr))
			response.FailWithMessage("Excel模板上传OSS失败", c)
			return
		}
		c.Header("X-Excel-OSS-URL", url.QueryEscape(fileURL))
		c.Header("X-Excel-OSS-Key", url.QueryEscape(key))
...
func (sysExportTemplateApi *SysExportTemplateApi) ImportExcel(c *gin.Context) {
	...
	fileURL, key, err := sysExportTemplateService.ImportExcelAndStore(templateID, file)
	if err != nil {
		global.GVA_LOG.Error(err.Error(), zap.Error(err))
		response.FailWithMessage(err.Error(), c)
	} else {
		response.OkWithDetailed(gin.H{"url": fileURL, "key": key}, "导入成功，Excel已归档到OSS", c)
	}
}
