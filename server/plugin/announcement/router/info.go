...
// Init 初始化 公告 路由信息
func (r *info) Init(public *gin.RouterGroup, private *gin.RouterGroup, authenticated *gin.RouterGroup) {
	{
	...
	{
		group := authenticated.Group("info")
		group.GET("notifications", apiInfo.Notifications)
		group.POST("read", apiInfo.MarkRead)
		group.POST("readAll", apiInfo.MarkAllRead)
		group.GET("stream", apiInfo.Stream)
	}
}
