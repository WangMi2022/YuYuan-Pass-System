...
- `asset-draft` Prompt V3 允许返回部分字段，并兼容 `productName`、`manufacturer`、`warrantyMonths` 三个受控别名；服务端仅在对应标准字段为空或为 0 时补位，最终只保留 `name`、`brand`、`recommendedWarrantyMonths` 标准字段。未知字段仍受 `additionalProperties: false` 拒绝。

仓库级验收：识别草稿与单次建档、标准化重复阻断、Schema 失败重试、别名归一、删除清理重试和权限迁移幂等均有自动化测试。生产运行验收已于 2026-08-14 通过：版本 `4ead10f0977adcb5020ed53442b0a93ca8e38032` 的 Prompt V3 为唯一活跃版本，真实 Vision 调用成功进入待确认草稿，调用审计记录 Prompt V3，正式删除接口完成任务软删除和临时图片清理，发布门禁 8/8 通过且容器重启数为 0。AI 不自动创建正式资产；1440、900、390 三档页面验收和更多真实铭牌样本校准仍待补充。
...
仓库级验收：字段差异生命周期、敏感值摘要、正式确认和质量聚合均有自动化测试。生产运行验收已于 2026-08-14 通过：修正表和关键质量字段已迁移，五个 `/invoiceQuality/*` 接口均返回业务码 `0`，发布门禁 8/8 通过且容器重启数为 0。验收时 11 张生产发票均未产生字段级复核样本，因此当前修正记录和字段指标为空是历史数据口径的真实结果；需从后续人工复核持续校准。1440、900、390 三档页面验收仍待补充。
...
- 只允许注册的 13 个只读 Tool，不接受任意 SQL、任意函数或模型生成写操作。
- Tool 执行前使用认证用户和角色确定数据范围；会话、消息和引用按用户隔离。
- Tool 列表按原业务 Casbin 权限过滤；权限校验失败发生在会话创建前，不产生空会话。
- 数据型回答必须保存查询范围、Tool、命中数、对象 ID 和跳转参数；模型摘要失败时返回确定性回答。
- 资产、风险、质保、保管人、流转、发票、日程和公告查询不得因自然语言改写而扩大权限。
- 资产条件查询支持日期、金额、状态、保管人/部门的 AND/OR 组合、排序、分页及分组汇总；模型只返回结构化条件，后端校验后绑定查询参数。资产总数和金额基于全部匹配记录，区分档案条数与实物数量。
- 已过保以 Asia/Shanghai 当前日期比较质保到期日；未知日期不视为已过保。单独“过期”、缺少金额阈值或不支持的条件先澄清，不能返回伪零结果。同一会话的质保口径补充保留原保管人范围。
- 维修次数仅统计未删除的已完成维修单，按业务日期过滤并去重；执行时同时检查资产与流转记录读取权限。资产字段和可用条件的完整约束见 [条件查询说明](SMART-ASSISTANT-P0-P3.md#资产条件查询)。

### 15.2 智能日报

- 指标快照由独立的确定性查询生成，包括资产流转和质保、风险、发票识别/复核/确认金额、个人日程、未读公告及 AI 失败率、耗时和费用。
- `user_id + authority_id + report_date` 唯一；重复或并发生成通过数据库 Upsert 更新当前角色的当天快照，不创建重复日报。
- 模型仅负责摘要，不得改变数字、筛选范围或跳转引用。
- 用户只能查看本人当前角色的日报，以及本人的订阅和投递记录；管理员菜单/API 权限仍由 Casbin 控制。
- 每分钟订阅任务按用户本地 `HH:MM` 发送站内通知或邮件，投递以用户、日报和渠道唯一并原子抢占发送状态，失败最多重试 3 次。

### 15.3 智能草稿

- 公告提取只接受已发布公告；同一用户和公告通过数据库幂等键复用现有日程草稿，过期后原位刷新，并保留日期、时间、地点、待办、原文和置信度。
- 业务草稿只能使用六类资产流转类型，资产 ID 必须存在且最多 100 项。
- 草稿确认通过 `draft → processing → accepted` 防止并发重复；过期、越权或已处理草稿拒绝确认。
- 日程确认只写入当前用户日历；资产确认只调用现有流转 Service 创建 `draft` 单，永不自动提交高风险动作。
- 候选查询、业务草稿生成和草稿确认会二次校验原日程或资产流转 Casbin 权限；业务 Service 继续负责状态机、资产锁定、事务和审计。
...
| --- | --- |
| 未登录访问智能接口 | HTTP `401` |
| 角色缺少 API 权限 | 权限拒绝，不泄露业务数据 |
| 查询本人会话/日报/草稿 | 成功 |
| 查询其他用户 ID 的会话/日报/草稿 | 不存在或无权访问 |
| 询问任意写操作 | 只读助手拒绝执行写入 |
| 模型关闭或超时 | 确定性结果/日报仍可用 |
| 重复提取同一公告 | 复用已有草稿 |
| 并发确认同一草稿 | 最多一个业务记录成功 |
| 草稿确认资产业务 | 只创建流转草稿，资产状态不立即改变 |
| 定时任务重复执行 | 投递记录不重复 |
