...
第二阶段已完成资产生命周期流转。尚未完成的盘点、财务及供应链模块不以空菜单形式展示，待对应业务模型和页面完成后分阶段开放。
...
分类管理下同时维护入库、使用、调入、归还、维修和处置六类位置字典。位置支持编码、说明、排序及启停管理；业务单仅加载对应类型的启用位置，同时允许录入不进入字典的临时自定义位置。单据保存位置文本快照，因此位置字典的停用或删除不会破坏历史记录。

### 3.4 资产流转

入库、领用、调拨、归还、维修和报废使用统一业务单据模型。单据包含业务类型、业务日期、目标位置、目标责任人、原因、备注和资产明细，并支持草稿与已完成状态。目标位置可从对应位置字典搜索选择，也可直接录入自定义值。

核心约束：

- 草稿不会修改资产，可编辑或删除。
- 提交时锁定相关资产，在同一数据库事务中校验状态并更新档案。
- 每次提交生成不可变流转记录，保存资产状态、位置和责任人变化前后的快照。
- 报废提交后资产转为已处置，当前估值归零。
- 已产生流转记录的资产档案不能删除。
- 当前版本以整条资产档案为流转单位，不支持同一档案数量的部分拆分流转。
...
监控状态按可配置周期采集 CPU、内存、磁盘、1/5/15 分钟系统负载、主机信息和 Go 运行时信息。页面保存最近 30 个浏览器端采样点，用于快速识别短时负载变化，并支持暂停和立即采集。
...
| 状态 | 含义 |
| --- | --- |
| 待入库 | 已建立档案并完成基础资料录入，尚未办理正式入库 |
| 闲置 | 资产可用但当前未投入使用 |
| 使用中 | 资产处于正常使用状态 |
| 维修中 | 资产正在维修或等待处理 |
| 已处置 | 资产已经报废、出售或移出管理范围 |

主状态按“建档 → 待入库 → 闲置 → 使用中/维修中 → 已处置”流转。调拨不改变主状态；归还将使用中或维修中的资产转回闲置。中间审批状态由业务单据承载，不写入资产主状态。
...
- 分别创建入库、领用、调拨、归还、维修和报废草稿，验证提交后的状态规则与流转快照。
- 检查大屏数量和金额与资产档案一致。
