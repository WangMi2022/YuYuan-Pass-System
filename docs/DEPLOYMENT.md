...
智能能力不从 `.env` 接收模型密钥。部署完成后使用管理员进入 `系统管理 → 智能服务 → 智能能力配置`，按“模型接入 → 识别服务 → 网关安全 → 配额计费 → Prompt 模板 → 运行监控”的顺序完成配置；API 响应只显示 `api-key-configured`，不会回显密钥。

模型单价和月预算在页面使用人民币元录入，允许小数；服务端仍以微单位保存，避免浮点误差。
...
2. 管理员可以在“系统管理 → 平台设置 → 登录外观”上传和恢复登录图标。
...
3. 六类生命周期单据均有已完成记录和 2 张可编辑草稿，报废资产当前估值为 0。
