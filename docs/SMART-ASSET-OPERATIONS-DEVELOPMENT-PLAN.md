...
> 文档状态：M0、M1、M2 已上线；M2 多端页面视觉验收待补
...
以下内容是本计划启动时的缺口基线。M0、M1 已补齐安全门禁与统一 Gateway；M2 已完成风险规则、事件闭环和扫描运行能力，并已通过生产运行验收；阈值校准和多端页面视觉验收在对应里程碑持续更新。
...
当前状态：已于 2026-08-14 发布生产。`release-acceptance.sh` 8/8 通过；四张风险表已迁移，17 条规则已预置，启动后的定时扫描成功，匿名总览和手动扫描接口均返回 401，Server/Web 容器重启数均为 0。真实资产命中质量、默认阈值、责任分配方式和管理员页面多端视觉验收仍需持续校准和补做。
...
生产运行验收：四张表成功迁移、17 条规则成功预置、匿名风险接口返回 401、启动后的定时扫描成功、容器重启数为 0，均已于 2026-08-14 通过。待补验收：管理员页面在桌面、平板和手机宽度下的视觉与交互检查。
...
| M2 风险中心 | Released / 生产运行验收通过 | 开发团队 | 2026-08-14 | 2026-08-14 | 10/10 | 默认阈值和生产命中质量待业务确认；多端页面视觉验收待补 |
...
M0、M1、M2 已上线，M2 生产运行验收已通过。下一阶段按以下顺序推进：

1. 结合生产命中数据校准 M2 的 17 条规则阈值和责任分配方式，并补做管理员页面多端视觉验收。
