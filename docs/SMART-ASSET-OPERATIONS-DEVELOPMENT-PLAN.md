...
> 文档状态：M0-M4 已发布生产；M3/M4 响应式修复已发布，M3 桌面双档通过，手机档与 M4 最终 UI 留证待补
...
以下内容是本计划启动时的缺口基线。M0、M1 已补齐安全门禁与统一 Gateway；M2 已完成风险规则、事件闭环和扫描运行能力，并已通过生产运行与多端页面验收；M3、M4 已完成生产发布、数据迁移、接口/模型运行链路验收和响应式修复。M3 已通过 1440、900 两档生产页面验收，M3 手机档、M4 三档与任务抽屉最终留证，以及真实样本持续校准仍在对应里程碑继续跟踪。
...
| M3 发票质量 | Released | 开发团队 | 2026-08-14 | 2026-08-14 | 7/7 | 后端与五个接口已验收；1440/900 UI 通过，字段级真实样本和 390 留证待补 |
| M4 智能建档 | Released | 开发团队 | 2026-08-14 | 2026-08-14 | 11/11 | Prompt V3、真实 Vision 和响应式修复已发布；更多样本、三档 UI 与抽屉留证待补 |
...
M0-M4 已发布生产。M3/M4 的数据库、权限、接口、模型调用、清理、响应式修复和发布门禁已通过验收；M3 的 1440、900 两档页面已通过，M3 手机档、M4 三档与任务抽屉最终留证，以及真实业务样本校准仍需继续。下一阶段按以下顺序推进：

1. 结合生产命中数据校准 M2 的 17 条规则阈值和责任分配方式。
2. 补齐 M3 的 `390×844` 手机档，以及 M4 的 `1440×1000`、`900×900`、`390×844` 三档页面和任务抽屉留证；发现问题立即修复并重新发布。
3. 使用真实复核发票和不同资产铭牌持续校准字段修改率、失败分布、置信度、单位回退和重复候选质量。
4. 启动 M5 只读业务助手，优先接入资产、风险和发票 Tool，保持引用、数据范围和只读边界。
5. 基于 M2-M5 的确定性数据实现 M6 智能日报，模型不可用时仍输出完整结构化指标。
