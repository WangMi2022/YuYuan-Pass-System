...
> 文档状态：M0-M4 已发布生产；M3 三档响应式结构与 M4 三档空态页面已通过，M4 有任务抽屉和截图归档待补
...
当前状态：功能已于 2026-08-14 随生产版本 `4ead10f0977adcb5020ed53442b0a93ca8e38032` 发布，响应式修复已随 `1a5e0f1e4829940cdd7886db6428c413753f74d5` 发布。生产 Prompt V1、V2 为 `retired`，V3 为唯一 `active`；真实照片任务通过正式重试接口从 `failed` 进入 `pending → processing → reviewing`，实际调用由 OpenAI Compatible Provider 的 `MiniMax-M3` 完成，耗时 5402 ms，任务和调用均记录 Prompt V3。结构化结果成功生成名称、品牌、型号、序列号、生产日期和 24 个月建议质保；模型未返回单位时草稿按确定性规则回退为“件”。验收结束后通过正式删除接口完成任务软删除并清理 1 张临时图片。AI 始终只生成待确认草稿，不自动创建正式资产。响应式修复覆盖 `<=1200px` 抽屉宽度、标题和底部按钮折行、移动端筛选与上传操作、任务面板根溢出和 OCR 长文本断行，并已通过定向 lint、生产构建、`git diff --check` 与发布门禁。空态页面在 `1440×1000`、`900×900`、`390×844` 三档的 `document/body.scrollWidth` 分别等于视口宽度，Header、刷新按钮、上传区、状态筛选和任务表格均在根页面内，没有发现页面级横向溢出。当前生产任务列表为空，任务抽屉宽高为 `0`，因此图片证据区、草稿表单、重复提醒和底部操作仍未完成有任务状态实测；CDP 截图持续超时，三档目前只有 DOM/几何证据。未配置 AI Gateway 或 Provider 时只影响新识别任务，普通资产新增与流转不受影响。
...
M0-M4 已发布生产。M3/M4 的数据库、权限、接口、模型调用、清理、响应式修复和发布门禁已通过验收；M3 三档响应式结构与 M4 三档空态页面已通过生产 DOM/几何验证。M4 有任务抽屉、截图归档和真实业务样本校准仍需继续。下一阶段按以下顺序推进：

1. 结合生产命中数据校准 M2 的 17 条规则阈值和责任分配方式。
2. 经单独确认后上传测试铭牌并触发一次生产 Vision 调用，补齐 M4 有任务抽屉的 `1440×1000`、`900×900`、`390×844` 三档验收，完成后通过正式接口清理任务和图片。
3. 截图能力恢复后补充 M3 手机档和 M4 三档归档图片；当前 DOM/几何证据与截图证据必须保持口径分离。
4. 使用真实复核发票和不同资产铭牌持续校准字段修改率、失败分布、置信度、单位回退和重复候选质量。
5. 启动 M5 只读业务助手，优先接入资产、风险和发票 Tool，保持引用、数据范围和只读边界。
6. 基于 M2-M5 的确定性数据实现 M6 智能日报，模型不可用时仍输出完整结构化指标。
