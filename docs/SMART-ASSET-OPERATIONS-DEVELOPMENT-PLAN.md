...
以下内容是本计划启动时的缺口基线。M0、M1 已补齐安全门禁与统一 Gateway；M2 已完成风险规则、事件闭环和扫描运行能力，生产验收结果在对应里程碑持续更新。
...
当前状态：仓库内实现与自动化验证完成，待生产发布、表和规则种子核验、权限与通知链路验收，以及按真实资产数据校准默认阈值。
...
生产待验收：四张表成功迁移、17 条规则成功预置、匿名风险接口返回 401、手动/定时扫描成功、页面在桌面和平板/手机宽度可用、容器重启数为 0。
...
| M2 风险中心 | Implemented / 待生产验收 | 开发团队 | 2026-08-14 | 待生产验收 | 10/10 | 默认阈值和生产命中质量待业务确认 |
...
M0、M1 已上线，M2 已完成仓库内实现。下一阶段按以下顺序推进：

1. 发布并验收 M2，结合生产命中数据校准 17 条规则阈值和责任分配方式。
