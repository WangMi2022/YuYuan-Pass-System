...
> 文档状态：M0-M4 已发布生产，M5-M7 代码与首轮交付文档已完成；M5-M7 生产权限、数据范围和发布验收待执行；M3/M4 证据归档仍有截图缺口
...
当前状态：M5 首版代码已完成，待部署环境完成数据库迁移、管理员菜单可见性和真实权限角色验收。
...
当前状态：M6 代码已完成，已增加每分钟订阅任务、站内通知、邮件发送、幂等投递和失败重试；生产 SMTP、日报指标口径和定时发送仍需部署环境验收。
...
当前状态：M7 代码已完成，公告编辑器已支持提取按钮，业务草稿已支持资产候选检索、人工确认和并发保护；多用户公告关联仍属于后续增强。
...
| M5 业务助手 | Implemented | 开发团队 | 2026-08-15 | 2026-08-15 | 首版+底层权限过滤完成 | 待部署权限和真实数据范围验收 |
| M6 智能日报 | Implemented | 开发团队 | 2026-08-15 | 2026-08-15 | 丰富指标+原子生成/投递完成 | 待生产 SMTP、指标口径和投递验收 |
| M7 智能草稿 | Implemented | 开发团队 | 2026-08-15 | 2026-08-15 | 详情确认+底层权限完成 | 多用户公告关联和生产草稿验收待补 |
...
M0-M4 已发布生产，M5-M7 首版代码已完成。M3/M4 的数据库、权限、接口、模型调用、清理、响应式修复和发布门禁已通过验收；M3 三档响应式结构与 M4 三档空态页面已通过生产 DOM/几何验证。M4 有任务抽屉截图归档、真实业务样本校准以及 M5-M7 的部署验收仍需继续。下一阶段按以下顺序推进：
...
4. 部署并验收 M5 业务助手，重点验证不同角色的资产、风险和发票数据范围。
5. 部署并验收 M6 智能日报的丰富指标、原子生成、站内通知和邮件失败/重试记录；API 与容器健康仍需接入独立可观测数据源。
6. 验收 M7 公告编辑器入口、资产候选、展开确认和底层业务权限二次校验，继续保持人工确认与不可自动提交边界。

这样先补完已发布能力剩余的 UI 留证和样本质量证据，再开放业务助手，避免在数据质量和权限 Tool 尚未验证时扩大 AI 访问面。
