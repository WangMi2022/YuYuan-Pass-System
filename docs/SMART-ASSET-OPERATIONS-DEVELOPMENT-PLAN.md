...
> 文档状态：M0-M4 已发布生产；M3/M4 后端与运行链路已验收，三档 UI 待补验
...
以下内容是本计划启动时的缺口基线。M0、M1 已补齐安全门禁与统一 Gateway；M2 已完成风险规则、事件闭环和扫描运行能力，并已通过生产运行与多端页面验收；M3、M4 已完成生产发布、数据迁移、接口/模型运行链路验收，三档 UI 验收和真实样本持续校准仍在对应里程碑继续跟踪。
...
当前状态：已于 2026-08-14 随生产版本 `4ead10f0977adcb5020ed53442b0a93ca8e38032` 发布。生产只读验收确认 `invoice_review_corrections`、`invoices.review_captured_at`、识别 Provider/模型/Prompt/耗时与多模态回退字段均存在；`/invoiceQuality/dashboard`、`providerMetrics`、`fieldMetrics`、`failures`、`classificationMetrics` 五个正式接口均返回业务码 `0`。验收时生产共有 11 张发票、11 个识别任务，字段级复核采集数和当前修正记录均为 0，因此字段指标和修正率必须从后续真实人工复核开始累计，历史发票不会推算不存在的字段差异。`release-acceptance.sh` 8/8 通过，Server/Web 容器重启数为 0；1440/900/390 三档页面视觉与交互验收仍待补充。
...
当前状态：已于 2026-08-14 发布生产版本 `4ead10f0977adcb5020ed53442b0a93ca8e38032`。生产 Prompt V1、V2 为 `retired`，V3 为唯一 `active`；真实照片任务通过正式重试接口从 `failed` 进入 `pending → processing → reviewing`，实际调用由 OpenAI Compatible Provider 的 `MiniMax-M3` 完成，耗时 5402 ms，任务和调用均记录 Prompt V3。结构化结果成功生成名称、品牌、型号、序列号、生产日期和 24 个月建议质保；模型未返回单位时草稿按确定性规则回退为“件”。验收结束后通过正式删除接口完成任务软删除并清理 1 张临时图片。AI 始终只生成待确认草稿，不自动创建正式资产。`release-acceptance.sh` 8/8 通过，Server/Web 容器重启数为 0；1440/900/390 三档页面视觉与交互验收仍待补充。未配置 AI Gateway 或 Provider 时只影响新识别任务，普通资产新增与流转不受影响。
...
| M3 发票质量 | Released | 开发团队 | 2026-08-14 | 2026-08-14 | 7/7 | 后端与五个接口已验收；字段级真实样本和三档 UI 待补 |
| M4 智能建档 | Released | 开发团队 | 2026-08-14 | 2026-08-14 | 11/11 | Prompt V3 与真实 Vision 已验收；更多样本和三档 UI 待补 |
...
M0-M4 已发布生产。M3/M4 的数据库、权限、接口、模型调用、清理和发布门禁已通过验收，但三档页面视觉与交互检查及真实业务样本校准仍需继续。下一阶段按以下顺序推进：

1. 结合生产命中数据校准 M2 的 17 条规则阈值和责任分配方式。
2. 补齐 M3/M4 的 1440、900、390 三档页面验收，发现问题立即修复并重新发布。
...
这样先补完已发布能力的 UI 和样本质量证据，再开放业务助手，避免在数据质量和权限 Tool 尚未验证时扩大 AI 访问面。
