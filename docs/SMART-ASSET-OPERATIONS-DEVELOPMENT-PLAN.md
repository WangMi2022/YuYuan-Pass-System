...
以下内容是本计划启动时的缺口基线。M0、M1 已补齐安全门禁与统一 Gateway；M2 已完成风险规则、事件闭环和扫描运行能力，并已通过生产运行与多端页面验收；阈值校准在对应里程碑持续更新。
...
当前状态：代码实现完成，等待生产数据库迁移、权限、真实数据口径和 1440/900/390 三档页面验收。历史发票不会推算不存在的字段差异，而是在看板中单独计入“历史无字段级修正数据”。
...
当前状态：代码实现完成，等待生产数据库迁移、Vision 配置、真实照片识别和 1440/900/390 三档页面验收。未配置 AI Gateway 或 Provider 时只影响新识别任务，普通资产新增与流转不受影响。
...
| M3 发票质量 | In Review | 开发团队 | 2026-08-14 | 2026-08-14 | 7/7 | 待生产迁移、真实数据和三档页面验收 |
| M4 智能建档 | In Review | 开发团队 | 2026-08-14 | 2026-08-14 | 9/9 | 生产需有效 Vision Provider、模型和图片白名单 |
...
M0、M1、M2 已上线并通过生产验收，M3、M4 已完成代码实现并进入发布验收。下一阶段按以下顺序推进：

1. 结合生产命中数据校准 M2 的 17 条规则阈值和责任分配方式。
2. 完成 M3/M4 数据迁移、权限、真实样本、三档页面和发布脚本验收。
3. 上线后观察发票字段修改率、失败分布、智能建档低置信字段和重复候选质量。
4. M3/M4 稳定后启动 M5 只读业务助手，优先接入资产与风险 Tool。
5. 最后基于 M2 和 M5 的确定性数据实现 M6 智能日报。

这样先把已完成的照片/铭牌识别与发票质量闭环稳定发布，再开放业务助手，避免在数据质量和权限 Tool 尚未验证时扩大 AI 访问面。
