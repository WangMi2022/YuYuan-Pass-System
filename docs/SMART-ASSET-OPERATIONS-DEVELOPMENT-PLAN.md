...
M0、M1、M2 已上线并通过生产验收。下一阶段按以下顺序推进：

1. 结合生产命中数据校准 M2 的 17 条规则阈值和责任分配方式。
2. 启动 M4 智能资产建档 MVP，复用 M1 的 Vision、Prompt、Schema、配额和审计能力。
3. 并行准备 M3 发票修正差异模型，先确保后续质量指标有可追溯数据源。
4. M4 稳定后启动 M5 只读业务助手，优先接入资产与风险 Tool。
5. 最后基于 M2 和 M5 的确定性数据实现 M6 智能日报。

这样先交付照片/铭牌识别的直接业务价值，再补发票质量闭环，并避免业务助手在数据与权限 Tool 尚未稳定时提前开放。
