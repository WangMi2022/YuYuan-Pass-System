...
开发或验收环境需要可重复的资产样例时，使用 `deploy/docker-dev/tools/seed-assets.sh`，不要把演示记录直接写入生产库。脚本会按当前资产域规则补齐预置分类、六类位置、资产档案和生命周期单据；生成数据带有可识别前缀，便于验收后清理。生产环境只应在已备份、受控的窗口执行，并在完成后核对清理结果。
