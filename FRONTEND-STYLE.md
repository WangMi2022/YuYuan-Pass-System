...
- 面向资产管理员、财务人员和内部运营人员的长时间桌面办公场景，整体保持专业、精确和克制。
- 当前任务、业务状态和可执行动作优先于装饰；高密度数据通过对齐、层级和一致控件保持可扫描。
- 自动识别和智能建议必须可解释、可复核、可撤销，权限、证据和审计记录属于业务流程本身。
- 日常办公态避免深色“指挥中心”、超大营销标题、装饰性卡片、彩虹图表、过度圆角，以及把关键复核状态藏进多层弹窗；专用会议室大屏可采用深石墨视觉，但仍遵循清晰层级和克制动效。
- 保持键盘可操作、清晰焦点、可读对比度、减少动态效果、响应式布局；状态表达不能只依赖颜色。
